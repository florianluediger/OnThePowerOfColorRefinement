...
\begin{Definition}
	Ein Graph $G$ wird \emph{amenable} genannt, wenn der Color Refinement Algorithmus diesen von jedem beliebigen, nicht zu $G$ isomorphen Graphen $H$ unterscheiden kann.
	\label{def:amenable1}
\end{Definition}

\begin{figure}[h]
	\centering
	\fbox{\includegraphics[width=0.75\textwidth]{./img/nicht_isomorphe_graphen.png}}
...
\subsection{Formale Definition und Amenability}
...
Aus den gewonnenen Erkenntnissen ergibt sich folgende Erweiterung zu Definition \ref{def:amenable1}.
\begin{Definition}
	Ein Graph $G$ wird \emph{amenable} genannt, wenn für jeden beliebigen, nicht isomorphen Graphen $H$ die Gleichung (\ref{eq:2}) für $i=n$ nicht erfüllt ist.
\end{Definition}